...
class EvalRunner:
	...
	def run(self)->list:
		print(f"[EvalRetrievalRunner] Running evaluation on {len(self.dataset)} queries...\n")
	...
	def _evaluate_single(self,item:dict)->dict:
		...
		retrieval_scores = response.get('retrieval_scores',None)
		reranker_scores_before = response.get('reranker_scores_before',None)
		reranker_scores_after = response.get('reranker_scores_after',None)
		guard_scores = response.get('guard_scores',None)
		...
		retrieval_score = [str(retrieval_scores)] if retrieval_scores is not None else []
		reranker_score_before = [str(reranker_scores_before)] if reranker_scores_before is not None else []
		reranker_score_after = [str(reranker_scores_after)] if reranker_scores_after is not None else []
		guard_score = [str(guard_scores)] if guard_scores is not None else []
		...
		'confidence':confidence,'rewrite_triggered':rewritten,'latency_seconds':latency,'answer':answer,'retrieval_score':retrieval_score,'reranker_score_before':reranker_score_before,'reranker_score_after':reranker_score_after,'guard_score':guard_score}
	...
	def _print_progress(self,result:dict):
		...
		print(f" {result['id']} | Query : {result['query']} | retrieved_ids: {result['retrieved_ids']} | Refusal:{result['refusal_correct']} | answer : {result['answer']} |retrieval_score:{result['retrieval_score']} |reranker_score_before:{result['reranker_score_before']} |reranker_score_after:{result['reranker_score_after']} |guard_score:{result['guard_score']}")
